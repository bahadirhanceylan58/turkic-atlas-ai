...
const HistoryEventCard: React.FC<HistoryEventCardProps> = ({
    ...
    return (
        ...
        >
            <div className={`bg-gradient-to-br ${importanceColors[event.importance || 'major']} bg-[var(--panel-bg)] backdrop-blur-xl border rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[inherit]`}>
                ...
                <div className="flex items-start justify-between px-4 pt-3 pb-2 flex-shrink-0">
                    ...
                    <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] p-1 rounded-lg hover:bg-[var(--surface-bg)] transition-all">
                        <X size={16} />
                    </button>
                </div>
                ...
                <div className="overflow-y-auto px-4 pb-4 flex-1 min-h-0 custom-scrollbar">
                    {/* Event Name */}
                    <h3 className="text-lg md:text-xl font-black text-[var(--text-primary)] mb-1 tracking-tight leading-tight">
                        {event.name}
                    </h3>
                    ...
                    {/* Divider */}
                    <div className="h-px w-full bg-gradient-to-r from-[var(--border-color)] via-[var(--border-color)] to-transparent mb-3" />

                    {/* Parties */}
                    {event.parties && event.parties.length >= 2 && (
                        <div className="flex items-center justify-center gap-3 mb-3 py-2 bg-[var(--surface-bg)] rounded-lg border border-[var(--border-color)]">
                            <span className="text-sm font-semibold text-blue-400">{event.parties[0]}</span>
                            <span className="text-amber-500 text-xs font-bold">VS</span>
                            <span className="text-sm font-semibold text-red-400">{event.parties[1]}</span>
                        </div>
                    )}

                    {/* Result */}
                    {event.result && (
                        <div className="flex items-center gap-2 mb-3 px-3 py-2 bg-[var(--surface-bg)] rounded-lg">
                            <Crown size={14} className="text-amber-500 flex-shrink-0" />
                            <span className="text-sm text-[var(--text-secondary)]">{event.result}</span>
                        </div>
                    )}

                    {/* Description */}
                    {event.description && (
                        <p className="text-xs md:text-sm text-[var(--text-secondary)] leading-relaxed mb-3 font-serif italic">
                    ...
                    {(dynastyInfo || isLoadingDynasty) && (
                        <div className="mb-3 p-3 bg-[var(--surface-bg)] rounded-lg border border-[var(--border-color)]">
                            <div className="flex items-center gap-1.5 mb-2">
                                <Crown size={12} className="text-amber-500" />
                                <span className="text-[10px] font-bold text-amber-500 uppercase tracking-wider">Hanedan</span>
                            </div>
                            {isLoadingDynasty ? (
                                ...
                                <p className="text-xs text-[var(--text-secondary)] whitespace-pre-wrap">{dynastyInfo}</p>
                            )}
                        </div>
                    )}
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[var(--accent-primary)]/20 to-blue-500/20 border border-[var(--accent-primary)]/30 text-[var(--accent-primary)] rounded-xl text-xs font-semibold hover:from-[var(--accent-primary)]/30 hover:to-blue-500/30 transition-all active:scale-95"
                    >
                        <Bot size={14} />
                        AI Analizi
                    </button>
